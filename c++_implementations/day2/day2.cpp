...
bool check_password_valid(std::string password, char letter, int min_times, int max_times)
{
    if (std::count(password.begin(), password.end(), letter) >= min_times &&
        std::count(password.begin(), password.end(), letter) <= max_times)
    {
        return true;
    }
    else
